...
async def workspace_symbols(
    ...
    """Search for symbols across a workspace environment by name.

    Args:
        query: Symbol name to search for (partial match supported)
        env_id: Environment ID to search in (from list_environments).
                Required if multiple environments exist, optional if only one.
        limit: Maximum items to return (default: 20)
        offset: Number of items to skip for pagination (default: 0)

    Returns {items, totalItems, hasMore, nextOffset} where each item has:
    - name: Symbol name
    - kind: Symbol type (5=Class, 12=Function, etc.)
    ...
    all_env_ids = list(mgr.environments.keys())

    if not all_env_ids:
        return {
            "error": "No environments discovered",
            "items": [],
            "totalItems": 0,
            "hasMore": False,
        }
    ...
    target_env_id: str
    if len(all_env_ids) == 1:
        # Single environment - use it automatically
        target_env_id = all_env_ids[0]
    elif env_id is None:
        # Multiple environments but none specified - error
        return {
            "error": "Multiple environments exist. Please specify env_id.",
            "available_environments": all_env_ids,
            "items": [],
            "totalItems": 0,
            "hasMore": False,
        }
    elif env_id not in all_env_ids:
        # Invalid env_id specified
        return {
    ...
    if ctx:
        await ctx.info(
            f"Searching workspace symbols in {target_env_id}: {query}"
        )

    # Get or start the client for this environment
    env = mgr.environments.get(target_env_id)
    if not env:
        return {
            "error": f"Environment not found: {target_env_id}",
            "items": [],
            "totalItems": 0,
            "hasMore": False,
        }

    # Start client if not already active
    if not env.client:
        try:
            await mgr._start_client(env)
        except Exception as e:
            logger.error(f"Failed to start client for {target_env_id}: {e}")
            return {
                "error": f"Failed to start pyright for environment: {e}",
                "items": [],
                "totalItems": 0,
                "hasMore": False,
            }
    ...
    try:
        ...
        logger.warning(f"Failed to query workspace symbols from {target_env_id}: {e}")
        return {
